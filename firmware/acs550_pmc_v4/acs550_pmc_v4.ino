/*
 * ============================================================================
 *  acs550_pmc_v4.ino   -- v4, adds a fan-speed analog output for the DAQ
 *
 *  Derived from acs550_pmc_v3.ino, which is UNTOUCHED at
 *  firmware/acs550_pmc_v3/ (and v2 at firmware/acs550_pmc/). The Modbus loop,
 *  both watchdogs, the control-word handshake, RD/WR and every existing
 *  telemetry field are unchanged. v4 only ADDS.
 *
 *  ---------------------------------------------------------------------------
 *  WHY v4 EXISTS
 *  ---------------------------------------------------------------------------
 *  Jeong's DAQ already records ROTOR rpm from a proximity sensor on the rig.
 *  It does not record FAN rpm, which lives only in this PMC. So the two halves
 *  of every experiment sit in two files with no common clock, and aligning
 *  them afterwards is guesswork.
 *
 *  One wire fixes that: fan speed as a 0-10 V analog signal the DAQ samples on
 *  the same time base as everything else it records.
 *
 *  ---------------------------------------------------------------------------
 *  WIRING -- PMC ANALOG OUT -> DAQ  (the v4 addition)
 *  ---------------------------------------------------------------------------
 *      PMC  O0   (first of the four ANALOG OUT screw terminals) --> DAQ AI+
 *      PMC  GND  (the one beside that block)                    --> DAQ AGND
 *
 *  Twisted pair, ferrules on both ends. Keep it away from the motor leads --
 *  they are the noisiest thing in the room and this is a millivolt-resolution
 *  signal.
 *
 *  Ground: land it on the DAQ's ANALOG ground, not chassis. PMC and DAQ may
 *  already share mains ground through the drive; if you see mains-frequency
 *  ripple on the channel, that is a ground loop and the fix is at the DAQ end.
 *
 *  ---------------------------------------------------------------------------
 *  THE SCALE -- live zero, and why
 *  ---------------------------------------------------------------------------
 *      volts = 0.5 + rpm / 300          valid reading
 *      volts = 0.0                      INVALID: no fresh speed from the drive
 *
 *           0 rpm -> 0.50 V        1200 rpm -> 4.50 V
 *         600 rpm -> 2.50 V        2435 rpm -> 8.62 V   (par 1105 max)
 *
 *  The 0.5 V offset is a "live zero", the same trick 4-20 mA uses. Without it,
 *  a stopped fan, a pulled wire, a dead PMC and a failed Modbus read all read
 *  0 V and are indistinguishable in the file. With it, 0 V means exactly one
 *  thing: DO NOT TRUST THIS SEGMENT.
 *
 *  This matters more than it looks. Everything on this rig that has ever gone
 *  wrong produced plausible numbers first. A stale fan speed held through a
 *  comm dropout would look like a perfectly good run.
 *
 *  RECORD THE SCALE OUTSIDE THE FIRMWARE. It is in data/tunnel.json. The DAQ
 *  only ever sees volts, so a constant that lives only here is one reflash
 *  away from silently invalidating every file ever captured.
 *
 ...
 *  ---------------------------------------------------------------------------
 *    ID            -> OK ID acs550-pmc 4.0 RD/WR AO
 *    AO?           -> OK AO <volts> <rpm> <valid> scale=0.5+rpm/300
 *    AOTEST <v>    -> force the analog output for calibration (30 s, idle only)
 ...
 *
 *  Telemetry line (v4 APPENDS two fields; older hosts zip-truncate and are
 *  unaffected):
...
// ---- v4: fan speed -> DAQ analog output -------------------------------
// Channel 0 of the PMC's ANALOG OUT block, terminal O0.
static const int      AO_CHANNEL    = 0;
static const uint8_t  AO_PERIOD_MS  = 4;      // PWM period; shorter = less
                                              // ripple if the library allows
static const float    AO_ZERO_V     = 0.5f;   // live zero -- 0 V means INVALID
static const float    AO_RPM_PER_V  = 300.0f; // 1200 rpm -> 4.50 V
static const float    AO_MAX_V      = 10.4f;  // hardware ceiling is ~10.5

// Par 0102 SPEED, read DIRECTLY rather than derived from output frequency.
// Frequency cannot give rpm without knowing slip, and assuming it does is
// exactly the error that made feedback read 295 where the drive held 300.
static const int      REG_ACT_SPEED = 101;    // par 0102 lives at register 101

static const uint32_t AOTEST_MS     = 30000;  // forced output lapses
...
static int16_t    actualRpm   = 0;       // v4: par 0102, true shaft rpm
static bool       rpmValid    = false;   // v4: false -> analog out reads 0 V
static float      aoVolts     = 0.0f;    // v4: what we last drove on O0
static uint32_t   aoTestUntil = 0;       // v4: manual override deadline
static bool       rpmWarned   = false;
...
// ===========================================================================
// v4: true shaft speed, and the analog output that carries it to the DAQ
// ===========================================================================

// Par 0102 SPEED is not in the 40004-40007 block -- that block carries
// 0103/0104/0106 via the EFB PAR 10-12 mapping, and remapping it would mean a
// group 53 change, which is keypad-only for good reason. One extra FC3 costs
// about 10 ms of a 100 ms cycle and needs no drive change at all.
//
// A failure here does NOT trip COMM_LOST. The control path is a separate
// transaction and may be perfectly healthy; only the DAQ signal is affected,
// and it says so itself by going to 0 V. Declaring the link dead -- and
// ramping down a running tunnel -- because one telemetry read failed would be
// a far worse outcome than a gap in a log.
static void readSpeed() {
  long v = ModbusRTUClient.holdingRegisterRead(SLAVE_ID, REG_ACT_SPEED);
  if (v < 0) {
    rpmValid = false;
    if (!rpmWarned) {
      Serial.println("# par 0102 read failed - analog out now reads 0 V "
                     "(INVALID); fan control is unaffected");
      rpmWarned = true;
    }
    return;
  }
  ...
  rpmValid  = true;
  if (rpmWarned) {
    Serial.println("# par 0102 read recovered, analog out live again");
    rpmWarned = false;
  }
}

// Drive O0. Called every poll, and on comm loss, so the DAQ channel is never
// left holding a stale value that looks like a real measurement.
static void updateAnalogOut() {
  if (aoTestUntil) {
    if (millis() < aoTestUntil) return;         // manual override still held
    aoTestUntil = 0;
    Serial.println("# AOTEST lapsed, analog out back to live fan speed");
  }

  float v;
  if (!rpmValid || state == ST_COMM_LOST) {
    v = 0.0f;                                   // the one unambiguous signal
  } else {
    // The fan does not reverse; a negative reading is a fault or a wrong
    // scale, and clamping it to zero rpm would hide that. Report INVALID.
    if (actualRpm < 0) v = 0.0f;
    else {
      v = AO_ZERO_V + (float)actualRpm / AO_RPM_PER_V;
      if (v > AO_MAX_V) v = AO_MAX_V;
    }
  }
  aoVolts = v;
  MachineControl_AnalogOut.write(AO_CHANNEL, v);
}
...
static void printTelemetry() {
  ...
  // v4 additions, appended so older hosts truncate rather than misparse.
  if (rpmValid) Serial.print(actualRpm); else Serial.print("nan");
  Serial.print(',');
  Serial.println(aoVolts, 3);
}
...
static void handleLine(char *line) {
  ...
  if (!strcmp(line, "PING")) {
    ...
    Serial.println("OK ID acs550-pmc 4.0 RD/WR AO");
  }
  ...
  else if (!strcmp(line, "AO?")) {
    Serial.print("OK AO "); Serial.print(aoVolts, 3);
    Serial.print(' ');      Serial.print(rpmValid ? actualRpm : 0);
    Serial.print(' ');      Serial.print(rpmValid ? "valid" : "INVALID");
    Serial.print(" scale=");Serial.print(AO_ZERO_V, 2);
    Serial.print("+rpm/");  Serial.print(AO_RPM_PER_V, 0);
    Serial.print(aoTestUntil ? " FORCED" : "");
    Serial.println();
  }
  else if (!strcmp(line, "AOTEST")) {
    // Forcing the output while the tunnel runs would write a lie into the
    // DAQ record of a live experiment. Calibrate on an idle rig.
    if (state == ST_RUNNING || state == ST_STARTING) {
      Serial.println("ERR refused - the fan is running; AOTEST would corrupt "
                     "the DAQ record");
    } else if (!arg) {
      Serial.println("ERR AOTEST needs a voltage");
    } else {
      float v = atof(arg);
      if (v < 0) v = 0;
      if (v > AO_MAX_V) v = AO_MAX_V;
      aoVolts     = v;
      aoTestUntil = millis() + AOTEST_MS;
      MachineControl_AnalogOut.write(AO_CHANNEL, v);
      Serial.print("OK AOTEST "); Serial.print(v, 3);
      Serial.print(" V for ");    Serial.print(AOTEST_MS / 1000);
      Serial.println(" s");
    }
  }
...
void setup() {
  ...
  // v4: analog out. Written to 0 V (INVALID) BEFORE anything else, so the
  // DAQ never sees a floating pin read as a plausible fan speed during boot.
  // NOTE: this is the Arduino_PortentaMachineControl API. The older
  // Arduino_MachineControl library spells it analog_out.write()/period_ms().
  MachineControl_AnalogOut.begin();
  MachineControl_AnalogOut.setPeriod(AO_CHANNEL, AO_PERIOD_MS);
  MachineControl_AnalogOut.write(AO_CHANNEL, 0.0f);

  Serial.println("# acs550-pmc 4.0 ready (RD/WR, fan rpm on analog out O0)");
  Serial.println("# T,t_ms,state,sp_hz,act_hz,amps,kw,sw,settled,fault,errs,"
                 "act_rpm,ao_v");
  Serial.println("# analog out O0: 0.5 V + rpm/300, and 0.000 V means INVALID");
...
void loop() {
  ...
  // v4: true shaft speed out to the DAQ. readSpeed() deliberately does not
  // feed noteComms -- see its comment. updateAnalogOut() runs unconditionally
  // so a comm loss drives 0 V rather than leaving the last good value.
  readSpeed();
  updateAnalogOut();
